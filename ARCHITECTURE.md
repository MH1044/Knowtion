...
Four gaps are known and deliberate rather than forgotten:
...
- **Packs are written non-durably.** `putIfAbsent` does a plain write, so a crash can
  leave a partial pack at a sequence number that then rejects forever and wedges the
  device. The advisory sidecars, by contrast, are written with fsync and an atomic
  rename — exactly backwards.
- **A failing write is silent.** Nothing in the UI distinguishes "saving normally" from
  "every write has failed since you opened the app".
